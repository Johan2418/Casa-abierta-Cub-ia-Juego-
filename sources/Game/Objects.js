...
export class Objects
{
    ...
    resetObject(object)
    {
        ...
        const body = object.physical && object.physical.body ? object.physical.body : null
        const isEnabled = body && typeof body.isEnabled === 'function' ? body.isEnabled() : null
        if(body && typeof body.setEnabled === 'function') body.setEnabled(false)
        if(body && typeof body.setTranslation === 'function') body.setTranslation(object.physical.initialState.position, false)
        if(body && typeof body.setRotation === 'function') body.setRotation(object.physical.initialState.rotation, false)
        if(body && typeof body.setLinvel === 'function') body.setLinvel({ x: 0, y: 0, z: 0 }, false)
        if(body && typeof body.setAngvel === 'function') body.setAngvel({ x: 0, y: 0, z: 0 }, false)
        if(body && typeof body.resetForces === 'function') body.resetForces()
        if(body && typeof body.resetTorques === 'function') body.resetTorques()

        // Wait a second and reactivate
        this.game.ticker.wait(1, () =>
        {
            if(body && typeof body.setEnabled === 'function' && isEnabled !== null) body.setEnabled(isEnabled)

            // Sleep
            if(object.physical.initialState && object.physical.initialState.sleeping && body && typeof body.sleep === 'function')
                body.sleep()
    ...
    disable(object)
    {
        ...
        {
            const b = object.physical.body
            if(typeof b.setLinvel === 'function') b.setLinvel({ x: 0, y: 0, z: 0 }, false)
            if(typeof b.setAngvel === 'function') b.setAngvel({ x: 0, y: 0, z: 0 }, false)
            if(typeof b.resetForces === 'function') b.resetForces()
            if(typeof b.resetTorques === 'function') b.resetTorques()
            if(typeof b.setEnabled === 'function') b.setEnabled(false)
        }

        if(object.visual && object.visual.object3D && typeof object.visual.object3D.removeFromParent === 'function')
    ...
    enable(object)
    {
        ...
        if(object.physical && object.physical.body && typeof object.physical.body.setEnabled === 'function')
            object.physical.body.setEnabled(true)

        if(object.visual && object.visual.parent && object.visual.object3D)
            object.visual.parent.add(object.visual.object3D)
    }
